...
import { getTopStoriesUrl, getStoryUrl, getStory } from '~/services/hackerNews'
...
const { data: topStoriesIds } = await useFetch(getTopStoriesUrl)

const loadStories = async (isServer) => {
  ...
  stories.value.push(...fetchedStories.map((story) => {
    const storyData = isServer ? story.data.value : story.data
